<template>
    <EditPaymentPanel v-if="isNewOrderPanelOpen" @close="isNewOrderPanelOpen = false; orderToEdit = null"
        :order="orderToEdit" />
    ...
    <div class="orders-content">
        <div class="topbar">
            <div class="title-page">Versements</div>
            <div class="actions">
        ...
        <div v-for="data in formattedData" :key="data._id">
            <div class="table-row">
...
const formattedData = computed<Order[]>(() => {
    ...
    const paymentList = Array.isArray(order.paymentsList) ? order.paymentsList.slice() : [];

    // --- FILTER ---
    const q = String(search.value ?? '').trim().toLowerCase();
    const filtered = q.length === 0 ? paymentList : paymentList.filter(o => {
        const parts: string[] = [];
...
const editOrder = (data: Order) => {
    ...
    const matched = order.paymentsList.find(o => {
