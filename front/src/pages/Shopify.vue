<template>
    <EditShopifyPanel v-if="isNewOrderPanelOpen" @close="isNewOrderPanelOpen = false; orderToEdit = null"
        :order="orderToEdit" />
    ...
    <div class="orders-content">
        <div class="topbar">
            <div class="title-page">Paiement Shopify</div>
            <div class="actions">
...
const formattedData = computed<Order[]>(() => {
    ...
    const shopifyList = Array.isArray(order.shopifyList) ? order.shopifyList.slice() : [];

    // --- FILTER ---
    const q = String(search.value ?? '').trim().toLowerCase();
    const filtered = q.length === 0 ? shopifyList : shopifyList.filter(o => {
        const parts: string[] = [];
