<template>
    <Panel>
        <div class="panel-title"> {{ title }} {{ props.order?._id }}</div>
...
const onSaveOrder = async () => {
    ...
    try {
        ...
        showToast('Versement ajouté', 'success')
    } catch (e) {
        apiErr.value = handleApiError(e)
    } finally {
...
const onUpdateOrder = async () => {
    ...
    try {
        ...
        await fetchOrders()
        const { showToast } = useToast()
        showToast('Versement mise à jour', 'success')
    } catch (e) {
        apiErr.value = handleApiError(e)
    } finally {
        isLoading.value = false
...
const title = ref<string>('Ajouter un versement')
...
onMounted(() => {
    if (props.order) {
        title.value = 'Modifier le versement'
        formOrder.value = {
            _id: props.order._id,
